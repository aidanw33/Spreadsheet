...
namespace SS
{
    ...
    public abstract class AbstractSpreadsheet
    {
        ...
        {
            //creates a linkedList to add all changed cell names
            LinkedList<string> changed = new LinkedList<string>();

            //create a hashSet which will store all visited cell names
            HashSet<string> visited = new HashSet<string>();

            //calls a recursive method which will find all dependent cells
            Visit(name, name, visited, changed);

            //return the cells which will need to be recalculated
            return changed;
        }
        ...
        /// 
        /// A recursive method which recusrvely calls cells names to add them to the changed list
        /// if they need to be recalculated
        /// </summary>
        private void Visit(string start, string name, ISet<string> visited, LinkedList<string> changed)
        {
            //add name to visisted
            visited.Add(name);
            
            //enumerate through all the dependents of name
            foreach (string n in GetDirectDependents(name))
            {
                //if this exists a circular dependency exists, throw error
                if (n.Equals(start))
                {
                    throw new CircularException();
                }
                //if we haven't visisted this cell yet, visit
                else if (!visited.Contains(n))
                {
                    Visit(start, n, visited, changed);
                }
            }
            //if cell needs to be recalculated, add it to changed
